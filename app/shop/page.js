    ...
    <main>
      ...
      <section style={{ backgroundColor: '#f5ebe0', padding: '50px 20px' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', gap: '25px', flexWrap: 'wrap', justifyContent: 'center', maxWidth: '1100px', margin: '0 auto' }}>
            {products.map(product => (
              <div key={product.id} style={{ backgroundColor: 'white', borderRadius: '15px', overflow: 'hidden', width: '300px', boxShadow: '0 4px 20px rgba(61,43,31,0.15)' }}>
                <div style={{ backgroundColor: '#c68642', height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '60px' }}>🕐</div>
                <div style={{ padding: '20px' }}>
                  ...
                  <h3 style={{ color: '#3d2b1f', fontSize: '18px', margin: '12px 0 8px', fontFamily: 'Georgia, serif' }}>{product.name}</h3>
                  <p style={{ color: '#6b4226', fontSize: '13px', lineHeight: 1.6, marginBottom: '15px' }}>{product.description?.substring(0, 90)}...</p>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '22px', fontWeight: 'bold', color: '#3d2b1f' }}>Rs.{product.price}</span>
                    <a href={"https://wa.me/919219265044?text=" + encodeURIComponent(product.whatsapp_message)} target="_blank" style={{ backgroundColor: '#25d366', color: 'white', padding: '8px 16px', borderRadius: '20px', textDecoration: 'none', fontSize: '13px', fontWeight: 'bold' }}>Order</a>
                  </div>
                </div>
              </div>
            ))}
